 ...
 * 
 */
public class TestOfWolf extends Actor
{
    GreenfootImage wolf_animation;
    GifImage wolf_Up = new GifImage("wolf-up-test.gif");
    GifImage wolf_Down = new GifImage("wolf-down-test.gif");
    GifImage wolf_Right = new GifImage("wolf-right-test.gif");
    GifImage wolf_Left = new GifImage("wolf-left-test.gif");
    ...
    public boolean isGameLost()
    {
        ...
        if ( nbEatenSheep ==3  || getWorld().getObjects(Farmer.class).isEmpty()) {
            //If TestOfWolf ate 3 Sheep (LifeParameter = 0), it's game over OR if TestOfWolf ate Farmer, it's instant game over
            return true;
        }
        else {
            return false;
    ...
    public static int getNbEatenSheep() { //count how many sheep the TestOfWolf ate
        return nbEatenSheep;
    }

    public int getLostPoints(){ // punish -50pts for every sheep TestOfWolf ate
